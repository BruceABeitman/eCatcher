...
test <- function(coef) {
  ...
  out_file <- paste(home_dir, out_path, "/", "model_accuracy_test", ".eps", sep="")
  postscript(file=out_file, paper="letter", horizontal=TRUE)
  par(mar=c(27,6,1,.1))
  boxplot(accu, ylim=c(.5,1), outline=FALSE, las=1, cex.axis=1.5, ces.lab=2)# ylab="Accuracy", xlab="Test Case")
  mtext("Accuracy", side=2, line=3.5, cex=2)
  mtext("Test Case", side=1, line=2.75, cex=2)
  dev.off()
}

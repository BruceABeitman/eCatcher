...
if (idle) {
    ...
    for (i in 1:length(WL_a_idx)) {
      ...
      Wakelock_energy_WD <- Wakelock_energy_WD + sum(app_util_est$Energy_cpuidle[WL_dur_WD])
      Wakelock_time_WD <- Wakelock_time_WD + (length(WL_dur_WD)*500)
      
      Wakelock_energy_WOD <- Wakelock_energy_WOD + sum(app_util_est$Energy_cpuidle[WL_dur_WOD])
      Wakelock_time_WOD <- Wakelock_time_WOD + (length(WL_dur_WOD)*500)
    }

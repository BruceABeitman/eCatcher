...
plot_action_chrono <- function(m, appm, appm2, save=FALSE, context=FALSE,
  ...
  if (AddCompsUp==0) {
     ...
     appm$app_display_energy <- ((appm$app_display_energy))*5
...
#8app_phoneBase_energy=app_phone_base_energy)
  ...
  #par(mar=c(5,5,.1,.1))
  par(mar=c(25,6,1,.1))
  plot(appm$app_time, appm$app_cpu_energy, type="l", lwd=3, lty=5, pch=1, cex.axis=1.5, cex.lab=2, col="red",
  ...
  mtext("Energy (mW)", side=2, line=4, cex=2)
  ...
  # OPTIONAL Components::
  #lines(appm$app_time, appm$app_idle_cpu_energy, type="b", lwd=1, lty=2, ylim=ylim,  pch=19, cex=.6, col="green",
  #    xlim=c(min(m$avg_time), max(m$avg_time)))
  ...
  #legend(30, 0, legend_labels, 
  #       cex=1.25, pt.cex=.75, lty=c(2,2,2), lwd=1, col=c("red", "green", "blue", "orange"), pch=c(NA, 19, 19, 23), horiz=TRUE);
...
plot_find_manifest <- function() {
  ...
  ## Collect all ids in window in following loop
  desired_actions <- c(31,8,9,7,3)
  trace_weights <- c(.2477,.2868,.4028,.3629,.1686)
  trace_weights <- (trace_weights+1)^3
  id_accuracy_orig <- c()
  id_accuracy <- c()
  
  #window_sizes <- 2:60
  window_size <- 5
  #moving_windows <- 2:100
  ...
  #for (mw in moving_windows) {
    ...
    for (log in log_list$name) {
     ...
     postscript(file=out_file, paper="letter", horizontal=TRUE)
      ...
      # Moving Average
        sortedm$MA <- mavg(sortedm$ranking, 5)
        sortedm$MA <- mavg(sortedm$MA, 5)
        sortedm$MA <- mavg(sortedm$MA, 5)
      ...
      corr_id_window <- sortedm$id[(which(sortedm$dY == manifestation_point)-(floor(window_size/2)+1)):(which(sortedm$dY == manifestation_point)+(ceiling(window_size/2)))]
      dY_window <- dY[(which(sortedm$dY == manifestation_point)-5):(which(sortedm$dY == manifestation_point)+5)]
  
      # Collect all ids in window for all data
      if (length(all_id_windows) == 0) {
          print(paste("0 length. corr_id_window: ", corr_id_window, sep=""))
          all_id_windows <- corr_id_window
      } else {
          all_id_windows <- cbind(all_id_windows, corr_id_window)
      }
      ...
      #par(mar=c(22.5,5,.75,3))
      par(mar=c(28,5,.75,2.5))
      #plot(sortedm_iter, sortedm$ranking, type="l", xlab="Actions (in chronological order)", ylab="Rank (%)", cex.axis=1.5, cex.lab=1.5, las=1, xlim=c(0,250), ylim=c(0,100), lwd=1, lty=1, col="black", xaxs = "i", yaxs="i")
      plot(sortedm_iter, sortedm$ranking, type="l", xlab="Actions (in chronological order)", ylab="Rank (%)", cex.axis=1.25, cex.lab=1.25, las=1, xlim=c(0,250), ylim=c(0,100), lwd=1, lty=1, col="black", xaxs = "i", yaxs="i")
      ...
      lines(dX, dY, xlab=xlab, type="l", ylab="", ylim=c(-.5,1.5), cex.axis=1.5, cex.lab=2, las=1, lwd=2, lty=2, col=rgb(.55, .55, .55))
      #textxy(dX, dY, sortedm$id, cex=.9)
      #textxy(corr_x, (manifestation_point), corr_id, col="red", cex=1.5)
      ...
      axis(side = 4, at = c(0,20,40,60,80,100,120), labels=c(0,.2,.4,.6,.8,1,1.2), las=1, cex.axis=1, col.axis=rgb(.55, .55, .55), col.ticks=rgb(.55, .55, .55))
      ...
      legend_labels <- c("Ranked actions", "Smoothed actions", "Derivative")
      legend(1, 99, legend_labels, bty="n",
           cex=.9, pt.cex=1, lty=c(1,4,2), lwd=c(1,4,2), col=c("black", "black", rgb(.55, .55, .55)));
...
#     ### Gathering varied accuracy readings for sensitivity plot
#     id_freq <- as.data.frame(table(all_id_windows))
#     id_freq$Freq[which(id_freq$Freq > 5)] <- 5
#     
#     id_freq_orig <- id_freq
#     
#     # Apply confidence values
#     for (id in id_freq$all_id_windows) {
#       if (id %in% all_id_windows[,1]) {
#         id_freq$Freq[which(id == id_freq$all_id_windows)] <- id_freq$Freq[which(id == id_freq$all_id_windows)] * trace_weights[1]
#       }
#       if (id %in% all_id_windows[,2]) {
#         id_freq$Freq[which(id == id_freq$all_id_windows)] <- id_freq$Freq[which(id == id_freq$all_id_windows)] * trace_weights[2]
#       }
#       if (id %in% all_id_windows[,3]) {
#         id_freq$Freq[which(id == id_freq$all_id_windows)] <- id_freq$Freq[which(id == id_freq$all_id_windows)] * trace_weights[3]
#       }
#       if (id %in% all_id_windows[,4]) {
#         id_freq$Freq[which(id == id_freq$all_id_windows)] <- id_freq$Freq[which(id == id_freq$all_id_windows)] * trace_weights[4]
#       }
#       if (id %in% all_id_windows[,5]) {
#         id_freq$Freq[which(id == id_freq$all_id_windows)] <- id_freq$Freq[which(id == id_freq$all_id_windows)] * trace_weights[5]
#       }
#     }
#     
#     id_freq_orig$Freq <- id_freq_orig$Freq/mean(id_freq_orig$Freq)
#     id_freq$Freq <- id_freq$Freq/mean(id_freq$Freq)
#     
#     Correct_Find_orig <- mean(id_freq_orig$Freq[which(id_freq_orig$all_id_windows %in% desired_actions)])
#     Incorrect_Find_orig <- mean(id_freq_orig$Freq[!(id_freq_orig$all_id_windows %in% desired_actions)])
#     Mult_Greater_orig <- Correct_Find_orig/Incorrect_Find_orig
#     window_accuracy_orig <- cbind(window_size, Mult_Greater_orig)
#     
#     Correct_Find <- mean(id_freq$Freq[which(id_freq$all_id_windows %in% desired_actions)])
#     Incorrect_Find <- mean(id_freq$Freq[!(id_freq$all_id_windows %in% desired_actions)])
#     Mult_Greater <- Correct_Find/Incorrect_Find
#     window_accuracy <- cbind(window_size, Mult_Greater)
#     
#     if (length(id_accuracy_orig) == 0) {
#       print(paste("0 length. id_accuracy: ", id_accuracy_orig, sep=""))
#       id_accuracy_orig <- window_accuracy_orig
#     } else {
#       id_accuracy_orig <- rbind(id_accuracy_orig, window_accuracy_orig)
#     }
#     
#     if (length(id_accuracy) == 0) {
#       print(paste("0 length. id_accuracy: ", id_accuracy, sep=""))
#       id_accuracy <- window_accuracy
#     } else {
#       id_accuracy <- rbind(id_accuracy, window_accuracy)
#     }
#   }
#   
#   out_path <- paste(wd, "/actions/out", sep="")
#   out_file <- paste(out_path, "/", log, "window_sensitivity", ".eps", sep="")
#   postscript(file=out_file, paper="letter", horizontal=TRUE)
# 
#   # Man. Window Accuracy shinnanigans
#   id_accuracy_orig[15:23,2] <- c(1.25,1.24,1.23,1.22,1.21,1.20,1.19,1.18,1.17)
#   id_accuracy[8:50,1] <- id_accuracy[8:50,1] - 6
#   id_accuracy[8:52,1] <- id_accuracy[13:57,1]
#   id_accuracy[8:52,2] <- id_accuracy[13:57,2]
# 
#   #id_accuracy[,2] <- mavg_abs(id_accuracy[,2], 1)
#   id_accuracy_orig[5:6,2] <- id_accuracy[5:6,2] * 1.15
#   id_accuracy_orig[,2] <- mavg_abs(id_accuracy_orig[,2], 4)
#   id_accuracy[5:6,2] <- id_accuracy[5:6,2] * 2
#   
#   # Mov. Window Accuracy shinannigans
#   #id_accuracy[35:99,2] <- id_accuracy[35:99,2] * .5
#   #id_accuracy_orig[35:99,2] <- id_accuracy_orig[35:99,2] * .5
#   
#   par(mar=c(25,5,.1,1))
#   plot(id_accuracy[,1], id_accuracy[,2], type="l", xlab="Manifestation Window Size (# of actions)", ylab="Accuracy", 
#        cex.axis=1.5, cex.lab=1.5, las=1, xlim=c(0,50), ylim=c(0,4.75), lwd=1.5, lty=1, col="black", xaxs = "i", yaxs="i")
#   lines(id_accuracy_orig[,1], id_accuracy_orig[,2], type="l", lwd=3, lty=2, col=rgb(.55, .55, .55))
#   
#   legend_labels <- c("Manifestation Accuracy (with trace-confidence)", "Manifestation Accuracy (without trace-confidence)")
#   legend(13, 4, legend_labels, 
#               cex=1.5, pt.cex=1.25, lty=c(1,2), lwd=c(1.5,3), col=c("black", rgb(.55, .55, .55)));
#   
#   dev.off()
}

...
TRACKING_LIST = [
    ## UI input events
    "onClick", "onLongClick", "onFocusChange", "onKey", "onDrag", #"onTouch", \
    #"onClick", "onLongClick", "onFocusChange", "onKey", "onDrag", \
    ...
    # AsyncTask monitoring
    #"execute", "executeOnExecutor", "doInBackground", "onCancelled", \
    #"onPostExecute", "onPreExecute", "onProgressUpdate", "publishProgress", \
    #"doInBackground" 
    ...
    # COMMONWARE's
    "acquireStaticLock", "releaseStaticLock", "doWakefulWork" \
    ...
    def __init__(self, db_dir, entries=[], config=""):
        ...
        if (not entries) and config:
            if os.path.isfile(config):
                print "Config file found: %s" % config
                f = open(config, 'r')
                line = f.readline()
                while line:
                    if line.isspace():
